...
(function () {
  ...
  // ---------- v3.9.6: Editorial copy — static HTML slots from live data ----------
  function renderEditorialCopy(d) {
    ...
    const metaDesc = `Dossier OSINT v${meta.version || '3.9.6'} — BLUF + ML forecast. ${convN} convocatorias activas en 5 macroregiones. Margen ONPE ${margen} votos al ${pctActas}, proyección ML ${margenMl} votos Fujimori (IC95 [${icLo}, ${icHi}]), P(F)=${pF} %.`;
    setMetaContent('meta[name="description"]', metaDesc);
    setMetaContent('meta[property="og:title"]', `Dossier OSINT — Manifestaciones Perú · v${meta.version || '3.9.6'} BLUF + ML`);
    ...
    if (footerAbout) {
      ...
      footerAbout.textContent = `Producto OSINT v${meta.version || '3.9.6'} preparado el ${genDate} para residentes y profesionales en las cinco macroregiones del Perú. Integra terreno, redes sociales y medios alternativos. Su propósito es informar, no movilizar. La información se contrasta exclusivamente con fuentes públicas verificadas.`;
    }
